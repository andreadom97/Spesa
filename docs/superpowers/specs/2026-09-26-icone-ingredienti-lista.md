# Icone ingrediente — catalogo (bozza da rivedere)

64 icone (70 → 68, latta e scatoletta confluite il 26/09 → 64 al gate del 26/09: kiwi, affettato, mais tolte; mozzarella confluita in formaggio), ~135 nomi coperti. Rivista da Andrea il 26/09: tenute tutte le aggiunte, tofu con ricotta. **Fonte** di ogni nome:
**P** = in produzione e usato in almeno un piatto o una lista (query del 26/09/2026),
**B** = `INGREDIENTI_BASE`, **F** = `formati-tipici`, **D** = estrazioni delle diete,
**A** = aggiunta mia (spesa italiana comune, nessun dato a supporto).
**Fam.** = icona di famiglia (più ingredienti diversi sulla stessa sagoma).
**★** = nel pilota di Claude Design.

## Ortofrutta (28)

| # | Icona | Nomi coperti | Fonte | Fam. |
|---|---|---|---|---|
| 1 | banana | banane | P B | |
| 2 | mela | mele | P B F | |
| 3 | pera | pere | P | |
| 4 | arancia | arance, mandarini | P B · A | |
| 5 | limone | limoni, lime | P B · A | |
| 6 | avocado | avocado | B | |
| 7 | zucchina | zucchine | P B F D | |
| 8 | melanzana | melanzane | B D | |
| 9 | peperone | peperoni | P B | |
| 10 | broccolo | broccoli, cavolfiore | B · A | sì |
| 11 | finocchio | finocchi | B | |
| 12 | sedano | sedano | P B | |
| 13 | pomodoro ★ | pomodori | P B D | |
| 14 | pomodorini (grappolo) | pomodorini | P B F D | |
| 15 | insalata (cespo) | insalata, lattuga, valeriana | B F D | sì |
| 16 | foglie | spinaci freschi, spinaci surgelati, rucola, bietole | P B D | sì |
| 17 | carota ★ | carote | P B F D | |
| 18 | patata | patate, patate dolci | P B F D | |
| 19 | cipolla | cipolle, scalogno | P B · A | |
| 20 | aglio | aglio | B | |
| 21 | fungo | funghi champignon, funghi | P · A | |
| 22 | zucca | zucca | P | |
| 23 | fagiolini | fagiolini | P D | |
| 24 | uva | uva | P | |
| 25 | fragola | fragole, frutti di bosco | A | sì |
| 26 | cetriolo | cetrioli | D | |
| 27 | ~~kiwi~~ (tolta al gate del 26/09) | kiwi | A | |
| 28 | erbe (rametto) | basilico, prezzemolo, menta, rosmarino | P D · A | sì |

## Carne e pesce (7)

| # | Icona | Nomi coperti | Fonte | Fam. |
|---|---|---|---|---|
| 29 | bistecca ★ | manzo, macinato di manzo, vitello, carne | B F · A | sì |
| 30 | cosciotto ★ | pollo, petto di pollo, tacchino, fesa di tacchino | P B F D | sì |
| 31 | salsiccia | salsiccia, würstel | A | sì |
| 32 | pesce ★ | salmone (fresco/surgelato), merluzzo (filetto/surgelato), branzino, orata | P B F D | sì |
| 33 | gambero | gamberi surgelati, gamberetti | B · A | |
| 34 | ~~affettato~~ (fetta) (tolta al gate del 26/09) | prosciutto crudo, prosciutto cotto, bresaola, speck | B F · A | sì |
| 35 | pancetta | guanciale, pancetta | P · A | sì |

## Latticini e uova (7)

| # | Icona | Nomi coperti | Fonte | Fam. |
|---|---|---|---|---|
| 36 | uovo ★ | uova | P B F | |
| 37 | latte ★ (brik) | latte | P B F | |
| 38 | yogurt (vasetto) | yogurt greco, yogurt bianco, yogurt, skyr | P B F D | sì |
| 39 | formaggio ★ (spicchio) | parmigiano, pecorino, grana, feta | P B F | sì |
| 40 | ~~mozzarella~~ (confluita in formaggio al gate del 26/09) | mozzarella, burrata | P B F D · A | |
| 41 | formaggio fresco (vaschetta) | ricotta, philadelphia, tofu | P B F | sì |
| 42 | burro (panetto) | burro | B | |

## Cereali e forno (9)

| # | Icona | Nomi coperti | Fonte | Fam. |
|---|---|---|---|---|
| 44 | pasta ★ | pasta, pasta integrale, pasta di semola, pasta di legumi | P B F D | sì |
| 45 | riso | riso, riso basmati | P B F D | |
| 46 | chicchi (spiga) | farro, orzo, cous cous, quinoa | P B F D · A | sì |
| 47 | avena (ciotola) | fiocchi di avena | P B D | |
| 48 | pane ★ (pagnotta) | pane, pane di segale | P B F D | |
| 49 | pancarré (fetta) | pane in cassetta, fette biscottate | B F | sì |
| 50 | biscotto | biscotti, crackers | P F | sì |
| 51 | farina (sacco) | farina 00, amido di mais, pangrattato | P B F D | sì |
| 52 | cornetto | brioche, cornetto | D · A | |

## Dispensa (17)

| # | Icona | Nomi coperti | Fonte | Fam. |
|---|---|---|---|---|
| 53 | olio (bottiglia) | olio extravergine, olio di semi | P B F | sì |
| 54 | ampolla | aceto balsamico, salsa di soia | B · A | sì |
| 55 | sale (saliera) | sale | B | |
| 56 | spezie (barattolino) | pepe, cannella, cumino, curry, paprika dolce, origano | B D | sì |
| 57 | zucchero (zollette) | zucchero | B | |
| 58 | miele | miele | B F | |
| 59 | marmellata | marmellata | F | |
| 60 | caffè (moka) | caffè | B | |
| 61 | latta pomodoro | passata di pomodoro, pelati | P B F | sì |
| 62 | scatoletta | tonno in scatola, tonno al naturale, sgombro | P B F · A | sì |
| 63 | ~~mais~~ (pannocchia) (tolta al gate del 26/09) | mais | B | |
| 64 | legumi ★ | ceci lessati, fagioli lessati, cannellini, borlotti, lenticchie (secche/lessate) | P B F | sì |
| 65 | piselli ★ (baccello) | piselli surgelati, edamame | P B F D | |
| 66 | noce | noci, frutta secca | P B F D | |
| 67 | mandorla | mandorle | P B | |
| 68 | arachide | burro di arachidi | P B | |
| 69 | cioccolato (tavoletta) | cioccolato fondente, gocce di cioccolato, cacao | P B F D | sì |

## Pronti e bevande (2)

| # | Icona | Nomi coperti | Fonte | Fam. |
|---|---|---|---|---|
| 70 | minestra (ciotola fumante) | minestrone surgelato, brodo vegetale | P B | sì |
| 71 | acqua (bottiglia) | acqua | A | |

## Decisioni della revisione (26/09)

- Tenute le aggiunte senza dati (fragola, kiwi, salsiccia, acqua).
- Surgelati senza icona propria: usano l'icona dell'ingrediente, il viola dell'area dice "surgelato".
- Tofu dentro "formaggio fresco".
- Esclusi di proposito: "frutta fresca", "verdura", "condimenti" (categorie, non ingredienti).

La numerazione salta il 43 (tofu, confluito nel 41).

## Esito del gate di Andrea (26/09, dopo la revisione)

- **kiwi**: via dal catalogo (nessuna icona per kiwi).
- **affettato**: via dal catalogo (prosciutto, bresaola, speck, mortadella, salame, affettato → nessuna icona).
- **mais**: via dal catalogo (mais → nessuna icona). "Amido di mais" resta su `farina`, è un sinonimo di farina.
- **mozzarella**: la chiave sparisce; i suoi sinonimi (mozzarella, burrata, stracciatella, fiordilatte) passano alla chiave `formaggio`.
- Tutto il resto resta com'è.
- Il catalogo passa da 68 a **64** chiavi.

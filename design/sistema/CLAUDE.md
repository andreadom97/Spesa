...
## Come si chiude una sessione: il delta
...
19/09 quel lavoro è costato un documento di ricostruzione da 75 KB
(`design/ridisegno/ANALISI.md`), e nessuno dei suoi numeri era nuovo.
...
- **Un delta che tocca un token tocca anche `tokens.css`.** L'ordine resta quello della regola
  d'oro del `README.md`: prima `DESIGN.md`, poi `tokens.css`, poi la scheda che lo mostra. Nel
  repo un test confronta `tokens.css` con `src/app/globals.css` e fallisce se lo stesso token
  vale due cose diverse: un valore dimenticato lì si vede subito.
